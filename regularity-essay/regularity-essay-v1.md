...
| How large is the *exceptional set*? | $\mathcal H^{n-1}$-measurable, $\sigma$-finite | $\dim_{\mathcal H}\operatorname{Sing}\le n-7$ (codim. 1) | $\dim_{\mathcal H}\operatorname{Sing}\le n-5$ (free bdy.), $\le 1$ (Navier–Stokes, parabolic) |
...
Let $\Omega\subset\mathbb R^n$ be open. A function $u\in L^1_{\mathrm{loc}}(\Omega)$ is of *bounded variation* if its distributional gradient $Du$ is an $\mathbb R^n$-valued Radon measure of finite total variation on compact subsets. The structure theorem (see [3,4]) decomposes

$$
Du \;=\; \nabla u\,dx \;+\; u\,D^c u \;+\; u^\sim\,D^s u ,
$$
...
where $\nu_E$ is the measure-theoretic outer normal. The point is that the perimeter is a *measure* — the total variation of the distributional derivative — and the "boundary" of $E$ is the support of this measure.
...
1. **Concentration.** The perimeter measure is carried by the reduced boundary: $|D\chi_E| = \nu_E\,\mathcal H^{n-1}\!\lfloor_{\partial^* E}$ (with the sign convention that $\nu_E$ is the *inner* normal; with the outer normal one gets $D\chi_E = -\nu_E\,\mathcal H^{n-1}\!\lfloor_{\partial^* E}$).
...
The central regularity statement (Allard, De Giorgi, Federer — see [5,7]) is an $\varepsilon$-regularity theorem:
...
- If the singular set had Hausdorff dimension $>k-d$ for some $d$, one could find a point whose tangent cone is singular in a $d$-dimensional family of directions, contradicting the (inductively known) regularity of cones of dimension $<k$.
- Induction on dimension yields $\dim_{\mathcal H}\operatorname{Sing}(V)\le k-d$.

In codimension one the sharp result is due to Federer [8]:
...
The *threshold* $m-7$ is where the PDE enters decisively. The dimension-reduction reduces the question to: *does there exist a non-flat stationary (or minimizing) cone with an isolated singularity?* The answer is:

- $m\le 6$: **no** — every such cone is a plane (De Giorgi $m=2,3$; Fleming; Almgren $m=3,4,5$; Simons [9] $m\le 6$). Hence $\operatorname{Sing}=\varnothing$ for $m\le 6$.
- $m=7$: **yes** — Simons [9] constructed the cone over $S^3\times S^3\subset\mathbb R^8$,
  $$\mathcal S=\{x\in\mathbb R^8: x_1^2+x_2^2+x_3^2+x_4^2=x_5^2+x_6^2+x_7^2+x_8^2\},$$
  which is *stable*; Bombieri–De Giorgi–Giusti [10] proved it is in fact *area-minimizing*, so it is a genuine minimizing hypersurface with an isolated singularity at the origin. This settles the Bernstein problem and shows the $m-7$ bound is **sharp** (for $m=7$, $m-7=0$).
- $m\ge 8$: products $\mathcal S\times\mathbb R^{m-7}$ give minimizers with $\mathcal H^{m-7}(\operatorname{Sing})>0$.

For *stable* (not necessarily minimizing) hypersurfaces the same $n-7$ bound holds (Schoen–Simon [12] and the 1981 lectures), and the singular set is *countably $(n-7)$-rectifiable with locally finite $(n-7)$-measure* (Simon; sharpened by Wang–White [13]). See also the survey of De Lellis.

**Takeaway.** GMT turns the qualitative "smooth outside a small set" into a *dimension-sharp* statement. The measure-theoretic object (current/varifold) is what provides compactness and makes the blow-up and dimension-reduction rigorous; the PDE (the minimal-surface equation, the second variation, the stability inequality) is what is actually used *at the tangent-cone level* to rule out (or allow) singular cones. The number $n-7$ is not geometric bookkeeping — it is the output of an elliptic-PDE computation (Simons' second-variation estimate on the second fundamental form).
...
$$
$u$ is harmonic in the free region $\{u>\varphi\}$ and *quadratic* (to leading order) in the contact set. Kinderlehrer–Nirenberg [20] and Kinderlehrer–Nirenberg–Spruck [21,22] proved that, under thickness/non-degeneracy conditions, the free boundary is real-analytic.
...
$$
Alt–Caffarelli–Friedman [23] introduced the *monotonicity formula*
$$
\Phi(r)=r^{-4}\int_{B_r^+}|\nabla u|^2 - r^{-2}\lambda_1^2|\Omega^+\cap B_r|
$$
that is non-decreasing in $r$ (up to a controlled error), forcing *uniqueness of the blow-up* at a free-boundary point.

**Caffarelli's program** [19,24,25] is the PDE counterpart of De Giorgi's GMT regularity:

1. *Blow-up.* Rescale about a free-boundary point; the monotonicity formula (Alt–Caffarelli–Friedman [23]; Weiss [26]) gives uniqueness of the blow-up, which is classified (half-plane, two-plane, or a degenerate quadratic).
2. *Flatness $\Rightarrow$ Lipschitz $\Rightarrow C^{1,\alpha}\Rightarrow$ analytic.* A compactness/linearization argument (Caffarelli [24,25]) upgrades flatness to full $C^{1,\alpha}$ regularity of the interface, and then to analyticity (Kinderlehrer–Nirenberg [20]).
3. *Singular set.* At points where the blow-up is a *degenerate* quadratic (the kernel of the Hessian has positive dimension), the interface may be singular. The regular part is $C^{1,\alpha}$; the singular part has *controlled dimension* — codimension at least $3$ for the obstacle problem (the optimal bound being a major open problem), and the structure is stratified by the rank of the degenerate blow-up.
...
- *Lions [28]* and *Lions–Magenes [29]* established the $L^2$ theory.
- The general $L^p$ result for the Laplacian on smooth domains was a long-standing open problem; the full *$L^p$ maximal regularity problem* (for arbitrary sectorial operators / Banach spaces) was solved by *Kalton–Weis [30]* using the $H^\infty$-functional calculus and $R$-boundedness.
- *Denk–Hieber–Prüss [31]* extended the theory to *operator-valued* coefficients via vector-valued Fourier multipliers.
- *Prüss [32]* gave the classical parabolic results; Amann's monograph develops the anisotropic / nonlinear theory.
...
The relevant energy is the **Modica–Mortola** functional [33,34]:
...
The **Modica–Mortola theorem** [34] (and the minimal-interface criterion of Modica [33]) is a $\Gamma$-convergence result:
...
The sharp-interface limit of this PDE is *motion by mean curvature* (the $L^2$-gradient flow of the perimeter). The rigorous convergence was proved by *Evans–Soner–Souganidis [35]* and *Barles–Soner–Souganidis [36]*: the zero level set $\{u_\varepsilon=0\}$ converges (in the sense of the associated *generalized* motion) to the generalized motion by mean curvature. The method is a *comparison principle* for the limiting *Hamilton–Jacobi* equation for the distance function — a PDE for the interface — combined with the *viscosity-solution* theory. The convergence holds *beyond the onset of singularities*, provided one uses the *generalized* (level-set) motion.
...
- *Chen [37]* obtained **first-order** $O(\varepsilon)$ estimates for the Hausdorff distance between the diffuse and sharp interfaces, via *comparison* (sub/supersolutions), valid *before the onset of singularities*.
- *Nochetto–Paolini–Verdi [38]* obtained **optimal second-order** $O(\varepsilon^2)$ estimates for a *thin / double-obstacle* formulation, by constructing explicit sub/supersolutions from the *asymptotic expansion* of $u_\varepsilon$.

The mechanism is exactly the interplay of the three faces. The sub/supersolutions are built from an *asymptotic expansion*
$$
u_\varepsilon(x,t) \;=\; \tanh\!\Big(\frac{d(x,t)}{2\varepsilon}\Big) + \varepsilon\,u_1(x,t)+O(\varepsilon^2),
$$
where $d(x,t)$ is the signed distance to the *sharp* interface. This expansion *requires the limit interface to be smooth* (the distance function $d$ must be differentiable, the curvature must be bounded). Where the limit develops a *singularity* (a corner, a cusp, a multiplicity point — the singular set of §3/§4), the expansion breaks down, the sub/supersolutions cease to exist, and the $O(\varepsilon^2)$ estimate fails. The *regular part* of the limit (the $C^{1,\alpha}$ / analytic free boundary of §4.2, the smooth part of the minimizing hypersurface of §3) is precisely where the numerical method is accurate; the *singular set* is precisely where it is not.
...
4. L. Ambrosio, N. Fusco, D. Pallara, *Functions of Bounded Variation and Free Discontinuity Problems*, Oxford University Press, 2000.
5. H. Federer, *Geometric Measure Theory*, Grundlehren der mathematischen Wissenschaften **153**, Springer, 1969.
6. H. Federer, W. H. Fleming, *Normal and integral currents*, Ann. of Math. (2) **72** (1960), 458–486.
7. W. K. Allard, *On the first variation of a varifold*, Ann. of Math. (2) **95** (1972), 417–491.
8. H. Federer, *The singular sets of area minimizing rectifiable currents with codimension one and of area minimizing flat chains modulo two with arbitrary codimension*, Bull. Amer. Math. Soc. **76** (1969), 767–771.
9. J. Simons, *Minimal varieties in Riemannian manifolds*, Ann. of Math. (2) **88** (1968), 62–105.
10. E. Bombieri, E. De Giorgi, E. Giusti, *Minimal cones and the Bernstein problem*, Invent. Math. **7** (1969), 243–268.
11. C. De Lellis, *A survey on the singular set of area-minimizing currents*, J. Differential Geom. (2023).
12. R. Schoen, L. Simon, *Stability of complete minimal surfaces and isolated singularities*, Invent. Math. **19** (1973), 223–232.
13. X.-P. Wang, B. White, *Singular set of stable minimal hypersurfaces*, Ann. of Math. (2) **187** (2018).
14. E. De Giorgi, *Sulla continuità delle estremali di certi funzionali del calcolo delle variazioni*, Ann. Mat. Pura Appl. (4) **37** (1957), 244–268.
15. J. Nash, *Continuity of solutions of parabolic and elliptic equations*, Amer. J. Math. **80** (1958), 931–954.
16. J. Moser, *A new proof of De Giorgi's theorem concerning the regularity problem for elliptic partial differential equations*, Comm. Pure Appl. Math. **13** (1961), 457–468.
17. S. Agmon, A. Douglis, L. Nirenberg, *Estimates near the boundary for solutions of elliptic partial differential equations satisfying boundary conditions of zeroth and first order*, Comm. Pure Appl. Math. **12** (1959), 623–727.
18. (See [17]; the $L^p$ elliptic estimates.)
19. L. A. Caffarelli, *The regularity of free boundaries in higher dimensions*, Acta Math. **139** (1977), 155–184.
20. D. Kinderlehrer, L. Nirenberg, *Regularity in free boundary problems*, Ann. Scuola Norm. Sup. Pisa Cl. Sci. (4) **4** (1977), 373–391.
21. D. Kinderlehrer, L. Nirenberg, J. Spruck, *Regularity in elliptic free boundary problems. I*, J. Analyse Math. **34** (1978), 86–119.
22. D. Kinderlehrer, L. Nirenberg, J. Spruck, *Regularity in elliptic free boundary problems. II. Equations of higher order*, Ann. Scuola Norm. Sup. Pisa Cl. Sci. (4) **6** (1979), 637–683.
23. H. W. Alt, L. A. Caffarelli, A. Friedman, *Variational problems with two phases and their free boundaries*, Trans. Amer. Math. Soc. **282** (1984), 431–461.
24. L. A. Caffarelli, *A Harnack inequality approach to the regularity of free boundaries. Part I: Lipschitz free boundaries are $C^{1,\alpha}$*, Rev. Mat. Iberoam. **3** (1987), 139–162.
25. L. A. Caffarelli, *A Harnack inequality approach to the regularity of free boundaries. Part II: Flat free boundaries are Lipschitz*, Comm. Pure Appl. Math. **42** (1989), 55–78.
26. G. S. Weiss, *Monotonicity formulas for the obstacle problem and an application to the regularity of the free boundary*, Comm. Partial Differential Equations **27** (2002), 1585–1603.
27. L. Caffarelli, L. Kohn, L. Nirenberg, *Partial regularity of suitable weak solutions of the Navier–Stokes equations*, Comm. Pure Appl. Math. **35** (1982), 771–851.
28. J.-L. Lions, *Équations et systèmes différentiels opérationnels appliqués aux problèmes d'évolution*, Dunod, Paris, 1961.
29. J.-L. Lions, E. Magenes, *Opérateurs différentiels non linéaires avec conditions aux limites de type Neumann*, Dunod, Paris, 1972.
30. N. J. Kalton, L. Weis, *A solution to the $L^p$ maximal regularity problem*, Math. Z. **235** (2000), 559–568.
31. R. Denk, M. Hieber, J. Prüss, *$R$-boundedness, Fourier multipliers and problems of elliptic and parabolic type*, Mem. Amer. Math. Soc. **166** (2003), no. 788.
32. J. Prüss, *Maximal regularity for parabolic problems*, J. Math. Soc. Japan **39** (1987), 441–460.
33. L. Modica, *The gradient theory of phase transitions and the minimal interface criterion*, Arch. Ration. Mech. Anal. **98** (1987), 123–142.
34. L. Modica, S. Mortola, *Un esempio di $\Gamma$-convergenza*, Boll. Un. Mat. Ital. B (8) **14** (1997), 285–299.
35. L. C. Evans, H. M. Soner, P. E. Souganidis, *Phase transitions and generalized motion by mean curvature*, Comm. Pure Appl. Math. **45** (1992), 1097–1123.
36. G. Barles, H. M. Soner, P. E. Souganidis, *Front propagation and phase field theory*, SIAM J. Control Optim. **31** (1993), 439–469.
37. X. Chen, *Convergence of Ginzburg–Landau equations with applications to anisotropic interface motions in crystals*, Differential Integral Equations **6** (1993), 1653–1676.
38. R. H. Nochetto, M. Paolini, C. Verdi, *Optimal interface error estimates for the mean curvature flow*, Ann. Scuola Norm. Sup. Pisa Cl. Sci. (4) **21** (1994), 193–212.
